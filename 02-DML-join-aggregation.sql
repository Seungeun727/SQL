...
-- Nautrl JOIN
-- 조건 명시 하지 않고, 같은 이름을가진 컬럼으로 JOIN 
SELECT first_name, department_name
...
-- sum: 합계
-- 급여의 한계 
SELCT sum(salary) FROM employees;


-- avg: 평균
-- 급여의 평균
SELECT avg(salary) FROM employees;
-- avg 함수는 null 값은 집계에서 제외 ---> 주의하자.
...
FROM employees; -- -> 단일그룹의 그룹함수가 아님 error
...
SELECT department_id, AVG(salary)
FROM employees 
WHERE AVG(salary) >= 7000 -- --> 집계가 실행 X
GROUP BY department_id;  -- --> 그룹 함수는 허가되지 않음 error
-- 177 줄: 집계 함수 실행 이전에 WHERE 절을 검사하기 때문에
-- 사용된 집계 함수는 WHERE 절에서 사용할 수 없다.
...
-- ROLLUP 함수에서 추출되는 Item Total과 함꼐
